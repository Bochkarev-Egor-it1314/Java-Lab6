...
1) Создана маркерная аннотация `@Invoke`, которая может применяться только к методам и доступна во время выполнения программы

`@Target(ElementType.METHOD)` - указывает, к чему можно применять эту аннотацию.

`@Retention(RetentionPolicy.RUNTIME)` - аннотация доступна только во время выполнения программы.

2) Реализован пример класса `InvokeClass`, содержащего несколько методов, часть из которых помечена аннотацией `@Invoke`.
   
3) Написан обработчик `InvokeProcessor`, использующий Reflection API:
+ получает все публичные методы переданного объекта,
+ проверяет наличие аннотации `@Invoke` у каждого метода,
+ автоматически вызывает подходящие методы.
  
4) В методе `Main`:
+ создаётся экземпляр класса,
+ вручную вызывается обычный метод для сравнения,
+ запускается обработчик, который находит и вызывает все методы с `@Invoke`.
...
1) Создана пользовательская аннотация `@Default`
- Указана цель применения: к классам `TYPE` и полям `FIELD`.
- Установлена политика удержания `RUNTIME`, чтобы аннотация была доступна во время выполнения.
- Добавлено обязательное свойство `value()` типа `Class<?>`, чтобы задавать "тип по умолчанию".

2) Аннотирован пример класса и поля
- Класс `DefaultClass` помечен аннотацией с `value = String.class`.
- Поле `someField` помечено с `value = Integer.class`.

3) Реализован обработчик аннотаций
- Используя рефлексию `getAnnotation(), isAnnotationPresent()`, программа проверяет наличие аннотации `@Default` у класса или поля.
- Если аннотация найдена — выводится имя указанного в ней класса.
- Добавлена проверка входных данных (на null, существование поля и т.д.).

4) Продемонстрирована работа в `Main`
- Вызваны методы обработчика для аннотированного класса и поля.
- Показана корректная обработка ошибок (отсутствие аннотации, неверное имя поля, null и др.).
...
1) Создана аннотация `@ToString`, применимая к классам и полям, с параметром `value()` типа перечисления (YES/NO), по умолчанию YES. Аннотация сохраняется во время выполнения `@Retention(RUNTIME)`.

2) Создано перечисление `ToStringMod`e с двумя значениями — YES и NO, чтобы типобезопасно задавать поведение аннотации.

3) Реализован утилитный класс `ToStringProcessor` с методом `processToString(Object obj)`, который:
- Проверяет, разрешено ли использовать кастомное представление для всего класса (если `@ToString(NO)` — возвращает стандартный `toString()`).
- Через рефлексию перебирает все поля объекта.
- Включает в результат только те поля, которые либо не помечены аннотацией, либо помечены как `@ToString(YES)`.
- Формирует строку вида `ClassName{field1=value1, field2=value2}`.

4) Протестировано на примере класса `ToStringClass`, где поле `password` аннотировано как `@ToString(NO)` — оно не появляется в итоговой строке.

5) В методе `Main` реализован дружелюбный интерфейс с проверкой ввода (имя, возраст, пароль) и демонстрацией работы решения.
...
1) Создаём аннотацию `@Validate` — с помощью `@interface`, указываем, что она применима к классам и другим аннотациям `@Target`, доступна во время выполнения `@Retention(RUNTIME)`, и содержит обязательный параметр `value` типа `Class<?>[]`.

2) Аннотируем пример класса — помечаем тестовый класс `ValidateClass` аннотацией `@Validate`, передавая в неё массив классов для "валидации" (например, String.class, Integer.class и т.д.).

3) Реализуем обработчик — в классе `ValidateProcessor` метод `processValidate` проверяет:
- Не null ли входной класс,
- Есть ли у него аннотация `@Validate`,
- Извлекает и выводит список классов из аннотации.
- При нарушении условий выбрасывает исключение с понятным сообщением.

4) Демонстрируем работу в `Main` — вызываем обработчик для аннотированного класса и выводим результат в удобочитаемом виде.
...
1) Создаём аннотацию `@Two` с помощью ключевого слова `@interface`. Указываем, что она применяется к типам `@Target(ElementType.TYPE)` и доступна во время выполнения `@Retention(RetentionPolicy.RUNTIME)`. Добавляем два обязательных параметра: `String first()` и `int second()`.

2) Аннотируем пример класса `TwoClass` этой аннотацией, передав конкретные значения: строку и число.

3) Реализуем обработчик аннотации `TwoProcessor`, который с помощью рефлексии проверяет, есть ли у переданного класса аннотация `@Two`, и если есть — выводит её параметры.

4) В главном методе `Main` демонстрируем работу обработчика через простое текстовое меню, запрашивая у пользователя действие, и обеспечиваем базовую проверку ввода.
...
1) Создана кастомная аннотация `@Cache` с параметром `value` (массив строк), применимая к классам и доступная во время выполнения.
   
2) Пример класса `Cache` помечен этой аннотацией с указанием кешируемых областей.

3) Написан обработчик `CacheProcessor`, который через рефлексию проверяет наличие аннотации у переданного класса и выводит список кешируемых областей или соответствующее сообщение, если их нет или аннотация не применена.

4) В методе `Main` демонстрируется корректная работа обработчика, включая проверку входных данных (например, на null).
...
Задача решается созданием тестового класса `InvokeTest`, который проверяет все аспекты работы аннотации `@Invoke`
...
Проверка наличия аннотации `@Invoke`
...
Для решения создается 2 класса: `TwoClass` и `InvalidTwoClass`.

Класс `InvalidTwoClass` содержит пустую строку как первый аргумент и отрицательное значение как второй аргумент

Класс `TwoClass` берётся из задания 1.5

Тестовый класс `TwoTest` содержит 3 теста:

Проверка пустой строки в свойстве first и отрицательного значения в свойстве second

Проверка корректной аннотации

Проверка отсутствия аннотации
